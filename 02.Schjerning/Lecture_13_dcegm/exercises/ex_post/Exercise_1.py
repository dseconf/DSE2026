...
def setup(beta=0.98, rho=0.5, sigma=0.2):
    """Create the finite-horizon calibration used by time iteration.

    ``sigma=0`` gives deterministic income; ``R`` is set to ``1/beta``.
    """
...
def solve_ti(par):
    ...
    sol.root_success = np.zeros(par.T-1,dtype=bool)
    sol.root_max_residual = np.zeros(par.T-1) + np.nan
    ...
    for t in reversed(range(0,par.T-1)):  # T-2, T-3, ..., 0 
            ...
            sol.root_success[t] = res.success
            sol.root_max_residual[t] = np.max(np.abs(res.fun))
            x1 = res.x.copy()
