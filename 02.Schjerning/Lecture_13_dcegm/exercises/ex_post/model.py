...
class model_bufferstock():
    """Normalized life-cycle buffer-stock model solved with EGM.

    Use ``life_cycle_setup()``, change economic parameters, then call
    ``create_grids()``, ``solve()``, and ``simulate()`` in that order.
    """
    ...
    def setup(self):
        """Set the infinite-horizon baseline and numerical defaults."""
    ...
    def life_cycle_setup(self):
        """Replace the baseline horizon and income growth with a life cycle."""
    ...
    def create_grids(self):
        """Construct shock quadrature and time-varying asset grids."""
        ...
        if par.lambdaa == 0: # no borrowing
            ...
            # a. allocate
            par.a_min = np.zeros(par.T) + np.nan
    ...
    def solve(self):
        """Solve backward using loop or vectorized EGM according to ``par.vec``."""
    ...
    def simulate (self):
        """Simulate normalized and level outcomes using the solved policies."""
